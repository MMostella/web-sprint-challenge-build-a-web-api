...
- [1] Reset the database to its original state executing `npm run resetdb`.
...
- [1] Install _nodemon_ as a development dependency that would not be used in production.
...
- [1] `[PUT] /api/actions/:id`
  - Returns the updated action as the body of the response.
  - If there is no action with the given `id` it responds with a status code 404.
  - If the request body is missing any of the required fields it responds with a status code 400.
- [1] `[DELETE] /api/actions/:id`
  - Returns no response body.
  - If there is no action with the given `id` it responds with a status code 404.

#### Middleware functions

- [1] Write at least two middleware functions for this API, and consume them in the proper places of your code.
...
- [1] Submit via Codegrade by pushing commits to your `main` branch.
...
1. The core features of Node.js and Express and why they are useful.
   Node:
   -query string: includes method to deal with query strings
   -path: deals with file paths
   Express:
   -Router: It maintains a stack of Layers. Assign the path and middleware function to it, then push it into the stack
...
1. Understand and explain the use of Express Routers.
   Express router is a class which helps us not just providing routing to our app but also can extend this routing to handle validation, handle 404 or other errors etc.
